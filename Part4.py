...
def starcheck(count):   #function to check if a star or a space is to be output
    if count>0:
        return '*'
    else:
        return ' '
...
if not studentuser: #if user is a staff member they are provided with a horizontal histogram and a vertical histogram of all the outputs they recieved from the loop
    ...
    largestvalue = max(progress,progressMT,notprogress,exclude) 
    print("\n---------------------------------------------------------------\nVertical Histogram")  #vertical histogram using 'total' from the horizontal histogram
    print(f"Progress {progress}| Trailing {progressMT}| Retriever {notprogress}| Excluded {exclude}")
    for _ in range(largestvalue):
        print(f"    {starcheck(progress)}           {starcheck(progressMT)}            {starcheck(notprogress)}           {starcheck(exclude)}")
        progress-=1
        progressMT-=1
        notprogress-=1
        exclude-=1
